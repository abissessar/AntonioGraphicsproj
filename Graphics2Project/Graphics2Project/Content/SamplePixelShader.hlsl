...
{
	float4 newnorm = normaltexture.Sample(SampleType, input.uv);
	newnorm = (newnorm*2.0f) - 1.0f;
	float3 tempnorm = normalize(input.normal);
	float4 temptan = normalize(input.tan);
	float4 tempbiTan = normalize(input.biTan);
	float3x3 TBN;
	TBN[0] = temptan.xyz;
	TBN[1] = tempbiTan.xyz;
	TBN[2] = tempnorm.xyz;
	newnorm = float4(mul(newnorm.xyz, TBN), 1.0f);
	float4 tex_color = shadertexture.Sample(SampleType, input.uv);

	float r = saturate(dot(normalize(-lightDirection.xyz), newnorm.xyz));
	...
	float Lightr = saturate(dot(dir.xyz, newnorm.xyz));
